...
- 长期记忆：重要偏好、项目背景等信息可保存到 `memory.json`，并自动注入系统提示词。
- 内置工具：文件列表、文件读取、文件写入、安全命令执行、打开文件、长期记忆保存、网页搜索、网页正文读取、当前时间、天气预报。
...
`openclaw_mini/config.py` 会先读取项目根目录的 `.env`，再从环境变量构造 `Config`。必填项是 `DEEPSEEK_API_KEY`；其他配置都有默认值，例如默认工作区是 `./workspace`，默认历史文件是 `./chat_history.json`，默认记忆文件是 `./memory.json`，默认最大推理轮数是 `8`。
...
内置工具集中定义在 `openclaw_mini/tools/local.py` 的 `build_local_tool_registry(workspace, memory_store)` 中。启动时 `main.py` 和 `gui.py` 都会调用它，把默认工作区和记忆存储传进去，得到同一套工具。
...
- `content`：要长期记住的简洁事实。
- `category`：记忆类别，例如 `preference`、`profile`、`project`、`environment`、`goal`。
- `importance`：重要性 `1-5`，默认 `3`。
- `source`：来源说明，默认 `conversation`。

实现方式：写入 `OPENCLAW_MEMORY` 指向的 JSON 文件。`ChatHistory` 会把当前记忆渲染进系统提示词，因此新的模型请求会带上这些长期信息。
...
OPENCLAW_MEMORY            可选，默认 ./memory.json
